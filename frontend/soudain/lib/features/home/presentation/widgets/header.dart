...
class _HeaderState extends State<Header> {
  ...
  void initState() {
    BlocProvider.of<UserDataBloc>(context).add(GetUserDataEvent(
      onError: (message) => showToast(message,
          position: ToastPosition.bottom, duration: Duration(seconds: 2)),
    ));
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    ...
    double loadingCardHorizontalMargin =
        sl<DeviceSizeAdapter>().getResponsiveSize(
      context: context,
      portraitSizeAdapter: SizeAdapter(
          isHeight: false,
          smallPorcentage: 16,
    ...
    double avatarSoudainSeparatorSize =
        sl<DeviceSizeAdapter>().getResponsiveSize(
      context: context,
      portraitSizeAdapter: SizeAdapter(
          isHeight: true,
        ...
        children: [
          ...
          Expanded(
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  Welcome(welcomeTextSize, soudainTextSize),
                  SizedBox(
                    height: avatarSoudainSeparatorSize,
                  ),
                  BlocBuilder<UserDataBloc, UserDataState>(
                    builder: (context, state) {
                      if (state is UserDataInitialState) {
                        ...
                        return Expanded(
                            child: UserInfo(
                                state.userDataModel.avatar,
                                state.userDataModel.name,
                                userAvatarWidth,
                                userNameTextSize,
                                avatarNameSeparatorSize));
                      } else if (state is SessionDoesNotExistState ||
                          state is UserDataDoesNotExistState) {
  ...
  Widget UserInfo(String image, String name, double avatarWidth,
      double userNameSize, double avatarNameSeparatorSize) {
    return Column(
      ...
      children: [
        ...
        SizedBox(
          height: avatarNameSeparatorSize,
        ),
        Text(
          name,
          maxLines: 1,
  ...
  Widget GoToLoginPageWidget(double signUpPadding) {
    return Container(
      padding: EdgeInsets.symmetric(vertical: signUpPadding),
      child: CommomButton(
        ...
        buttonTextSize: 20,
        buttonPadding: 8,
        buttonFunction: () => BlocProvider.of<NavigationBloc>(context)
            .add(HomeToLoginNavigationEvent()),
      ),
    );
  }
}
