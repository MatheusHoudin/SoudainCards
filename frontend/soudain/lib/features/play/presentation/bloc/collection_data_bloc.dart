...
class CollectionDataBloc
    extends Bloc<CollectionDataEvent, CollectionDataState> {
    ...
    if (event is GetCollectionsData) {
      ...
      yield* result.fold((failure) async* {
        ...
        if (failure is NoInternetConnectionFailure) {
          yield CollectionDataError(message: noInternetConnection);
        } else {
          yield CollectionDataError(message: unexpectedServerError);
        }
      }, (result) async* {
        print(result);
        if (result.length > 0) {
          yield LoadedCollections(collections: result);
        } else {
