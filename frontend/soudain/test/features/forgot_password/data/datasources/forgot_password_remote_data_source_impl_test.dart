...
void main() {
  ...
  final dynamic request = {email: email};
  setUp(() {
  ...
  void setUpRequestPasswordResetSuccessful() {
    when(mockDio.post(any, data: request))
        .thenAnswer((_) async => Response(statusCode: 201, data: {
              "code": 201,
              "data": json.decode(
                  convertJsonToString('forgot_password_confirmation.json')),
              "message": "message"
            }));
  }

  test('Should call the right endpoint when request a password reset',
      () async {
  ...
  test(
      ...
      () async {
    when(mockDio.post(any, data: request)).thenThrow(DioError(
        response: Response(
            statusCode: 404,
            data: {"code": 404, "error": "error", "message": "error"})));
  ...
  test(
      ...
      () async {
    when(mockDio.post(any, data: request)).thenThrow(DioError(
        response: Response(statusCode: 400, data: {
      "code": 400,
      "error": [
    ...
    expect(() => call(email),
        throwsA(TypeMatcher<PasswordResetRequestMalformedException>()));
  });

  test('Should throw a ServerException when call to the server fails',
      () async {
    when(mockDio.post(any, data: request)).thenThrow(DioError(
        response: Response(
            statusCode: 500,
            data: {"code": 500, "error": "error", "message": "error"})));
