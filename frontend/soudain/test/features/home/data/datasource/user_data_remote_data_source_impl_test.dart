...
void main() {
  ...
  setUp(() {
    sessionModel =
        SessionModel.fromJson(json.decode(convertJsonToString('session.json')));
    userDataModel = UserDataModel.fromJson(
        json.decode(convertJsonToString('user_data.json')));
    mockSessionBox = MockSessionBox();
    mockDio = MockDio();

    dataSource =
        UserDataRemoteDataSourceImpl(dio: mockDio, sessionBox: mockSessionBox);
  });

  test('Should return the UserDataModel with the data from the api', () async {
    when(mockSessionBox.getSessionModel()).thenReturn(sessionModel);
    //when(mockSessionBox.getSessionModel().getToken).thenReturn("token");
